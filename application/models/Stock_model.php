...
class Stock_model extends CC_Model { 
	...
	public function store_merchant_acc($data) { 
        $this->db->insert('tbl_merchant_account', $data); 
        return $this->db->insert_id(); 
    }
	...
	public function stock_info($page='1',$search="") {
		...
		if($search!=""){ 
			$where .= ' and merchant_name LIKE "%'.trim($search).'%" OR product_name LIKE "%'.trim($search).'%"  ';  
		}
	...
	public function stock_count($search=''){
		 $this->db->select('*')
                ->from('tbl_stock')
				->where('deletion_status',0)
				->order_by('id', 'desc');
    ...
    public function merchant_account_info($page='1',$search="") {
		$PAGINATION=10;
		...
		if($search!=""){ 
			$where .= ' and company_name LIKE "%'.trim($search).'%" OR company_mobile LIKE "%'.trim($search).'%"  ';  
		}
  	...
  	public function getpaymentlist($bill_id){
		$result = $this->db->get_where('tbl_merchant_payment', array('bill_id' => $bill_id));
        return $result->result_array();
	}	
	
	public function store_payment($payment){ 
        $this->db->insert('tbl_merchant_payment', $payment); 
        return $this->db->insert_id(); 
    }
	...
	public function get_merchant_acc_by_id($stock_id) {
        $result = $this->db->get_where('tbl_merchant_account', array('id' => $stock_id, 'deletion_status' => 0));
        return $result->row_array();
    }
	...
	public function update_merchant_acc($merchant_acc_id, $data) {
        $this->db->update('tbl_merchant_account', $data, array('id' => $merchant_acc_id));
        return $this->db->affected_rows();
    }
	...
	public function remove_merchant_stock_by_id($stock_id) {
        $this->db->update('tbl_merchant_account', array('deletion_status' => 1), array('id' => $stock_id));
		//echo $this->db->last_query();exit();
        return $this->db->affected_rows();
    }
